...
embed_disp_rat_community <- function(g,coreNode){
  ...
  for (i in 1:length(V(subgraph_5))) {
    if(V(subgraph_5)[i]!=coreNodeinsubgraph5){
      embed = embeddedness(subgraph_5,coreNodeinsubgraph5,V(subgraph_5)[i])
      #disp = dispersion(subgraph_5,coreNodeinsubgraph5,V(subgraph_5)[v])
    
      if(embed > embed_max){
        embed_max = embed 
        maxembed_node = V(subgraph_5)[i]
      }
      if(disp > disp_max){
        disp_max = disp
        maxdisp_node = V(subgraph_5)[i]
      }
      if (embed>0){
        rat = ratiodispembed(disp,embed)
        if(rat>rat_max){
          rat_max = rat
          maxrat_node = V(subgraph_5)[i]
        }
      }
    }
  }
